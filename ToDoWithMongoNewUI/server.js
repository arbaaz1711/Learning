...
const getData = async (filter = false) => {
  ...
  for await (const doc of cursor) {
    console.log(doc, "3333");
    tasks.push(doc);
  }
  console.log(tasks, "tasks111");
...
// Handle GET requests to filter the tasks by priority
app.get("/:priority", async (req, res) => {
  const priority = req.params.priority; // Get priority value from URL params
  console.log("GET priorities", priority);
  
  // Validate priority values
  const validPriorities = ['low', 'medium', 'high'];
  if (!validPriorities.includes(priority)) {
    return res.redirect("/"); // Redirect to home if invalid priority
  }
  
  const filteredList = await getData({ priority });
  console.log(filteredList, "filtereddd");
  if (filteredList) {
    // Render the index.ejs page with the fetched tasks
    res.render("index", { todoList: filteredList, currentFilter: priority });
  }
});

// Handle POST requests to filter the tasks
...
// API endpoint for AJAX filtering - returns JSON data
app.get("/api/filter/:priority", async (req, res) => {
  const priority = req.params.priority; // Get priority value from URL params
  console.log("API filter priorities", priority);
  
  try {
    let filteredList;
    
    if (priority === "all") {
      filteredList = await getData(); // Get all tasks
    } else {
      // Validate priority values
      const validPriorities = ['low', 'medium', 'high'];
      if (!validPriorities.includes(priority)) {
      ...
      filteredList = await getData({ priority });
    }
    
    console.log(filteredList, "API filtereddd");
    
    // Return JSON data instead of rendered HTML
    res.json({ 
      success: true,
      data: filteredList,
